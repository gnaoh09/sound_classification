...
for idx, row in mfcc_data.iterrows():
    features = row['MFCC']  # or 'PCA_Features' if you've changed to PCA
    if not isinstance(features, (list, np.ndarray)) or len(features) == 0:
        print(f"Ignoring {row['File Name']}: Invalid or empty feature data.")
        continue
    X.append(features)
...
# Define CNN model
model = Sequential()
model.add(Conv2D(32, (3, 3), activation='relu', input_shape=(626,40,1)))
...
# Train the model   
history= model.fit(X_train_cnn, y_train, epochs=150, batch_size=32, validation_data=(X_test, y_test))
model.save('D:/HUST/dev/py/sound_classification/model/sound_classification_1709.keras')
#print(model.summary())
